    ...
    <div class="responsive-two-col">
      <div>
        <div class="search-bar" style="margin-bottom:1rem">
          ...
          <select id="med-cat" class="form-control" style="max-width:220px">
      ...
      <div class="cart-sidebar" id="cart-panel">
        ...
        <div class="flex justify-between mb-1">
          ...
          <span id="cart-total" style="color:var(--success);font-weight:700">Rs 0.00</span>
        </div>
...
async function loadMedicines() {
  ...
  try {
    ...
    if (!meds.length) {
      container.innerHTML = '<div class="empty-state"><div class="icon">Rx</div><p>No medicines found.</p></div>';
      return;
    }
    container.innerHTML = `<div class="medicine-grid">${meds.map(m => `
      <div class="medicine-card">
        <div class="badge badge-secondary" style="margin-bottom:0.5rem">${escapeHtml(m.category)}</div>
        <h4>${escapeHtml(m.name)}</h4>
        <p style="font-size:0.78rem;color:var(--text-muted)">${escapeHtml(m.unit)}</p>
        <div class="price">Rs ${m.price.toFixed(2)}</div>
...
function renderCart() {
  ...
  itemsEl.innerHTML = cart.map(item => `
    <div class="cart-item">
      <div>
        ...
        <div style="color:var(--text-muted);font-size:0.78rem">Rs ${item.price} x ${item.qty}</div>
      </div>
      <div style="display:flex;align-items:center;gap:0.5rem">
        <span style="color:var(--success);font-weight:600">Rs ${(item.price * item.qty).toFixed(2)}</span>
        <button class="btn btn-danger btn-sm remove-cart-btn" style="padding:2px 8px" data-id="${item.medicine_id}">x</button>
      </div>
    </div>`).join('');
  ...
  totalEl.textContent = 'Rs ' + State.cartTotal().toFixed(2);
}
...
async function renderMyOrders(el) {
  ...
  try {
    ...
    if (!orders.length) {
      listEl.innerHTML = '<div class="empty-state"><div class="icon">Rx</div><p>No orders yet. Visit the Pharmacy.</p></div>';
      return;
    }
    const statusBadge = s => {
      const m = { processing: 'badge-warning', shipped: 'badge-info', delivered: 'badge-success' };
      return `<span class="badge ${m[s] || 'badge-secondary'}">${escapeHtml(s)}</span>`;
    };
    listEl.innerHTML = `<div class="appointment-list">${orders.map(o => `
      <div class="appointment-card" style="grid-template-columns:1fr">
        ...
        <p>Tracking: <strong>${escapeHtml(o.tracking_id)}</strong></p>
        <div style="font-size:0.82rem">
          ${o.items.map(i => `<span class="tag-pill" style="margin:2px">${escapeHtml(i.name)} x${i.qty}</span>`).join('')}
        </div>
        <div class="flex justify-between w-full">
          <span style="font-size:0.82rem;color:var(--text-muted)">${escapeHtml(o.address)}</span>
          <span style="font-weight:700;color:var(--success)">Rs ${o.total_price.toFixed(2)}</span>
        </div>
        <div style="width:100%;background:var(--border);border-radius:4px;height:6px;overflow:hidden">
          <div style="height:100%;background:linear-gradient(90deg,var(--primary),var(--accent));width:${o.status === 'delivered' ? '100%' : o.status === 'shipped' ? '60%' : '20%'};transition:width 0.5s"></div>
        </div>
      </div>`).join('')}</div>`;
  } catch (err) {
    document.getElementById('orders-list').innerHTML = `<p style="color:var(--danger)">${escapeHtml(err.message)}</p>`;
  }
}
...
function renderEmergency(el) {
  el.innerHTML = `
    <div class="page-header">
      ...
      <p>Describe symptoms and get a quick risk guide. This is not a medical diagnosis.</p>
    </div>
    <div class="guide-panel" style="max-width:700px">
      <div class="form-row">
        <div class="form-group">
      ...
      <div style="margin-bottom:1rem">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:0.4rem">
          ${['chest pain', 'shortness of breath', 'high fever', 'severe headache', 'nausea', 'cough', 'back pain', 'fatigue', 'seizure', 'unconscious']
            .map(s => `<button class="slot-btn symptom-tag" type="button" style="font-size:0.75rem">${s}</button>`).join('')}
        </div>
      </div>
...
async function checkEmergency() {
  ...
  const age = parseInt(document.getElementById('em-age').value, 10);
  const gender = document.getElementById('em-gender').value;
  if (!symptoms) {
    toast('Please describe your symptoms.', 'error');
    return;
  }
  ...
  try {
    ...
    resultEl.innerHTML = `
      <div class="emergency-result ${cls}">
        <p class="eyebrow">${data.is_emergency ? 'Emergency attention advised' : 'Regular care suggested'}</p>
        <div class="result-big" style="color:${data.is_emergency ? 'var(--danger)' : 'var(--success)'}">${escapeHtml(data.severity)} Risk</div>
        ...
        ${data.matched_emergency.length ? `<div style="margin-top:1rem"><p class="form-label" style="color:var(--danger)">Emergency symptoms detected:</p>
          <div style="display:flex;flex-wrap:wrap;gap:0.4rem;margin-top:0.4rem">${data.matched_emergency.map(k => `<span class="badge badge-danger">${escapeHtml(k)}</span>`).join('')}</div></div>` : ''}
        ${data.matched_non_emergency.length ? `<div style="margin-top:0.75rem"><p class="form-label" style="color:var(--success)">Non-emergency symptoms:</p>
          <div style="display:flex;flex-wrap:wrap;gap:0.4rem;margin-top:0.4rem">${data.matched_non_emergency.map(k => `<span class="badge badge-success">${escapeHtml(k)}</span>`).join('')}</div></div>` : ''}
